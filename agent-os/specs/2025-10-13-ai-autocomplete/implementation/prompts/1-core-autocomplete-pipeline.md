We're continuing our implementation of AIAutoComplete by implementing task group number 1:
...
- [ ] 1.0 Establish configuration and debounce foundation
  - [ ] 1.1 Write config values, debounce timing, whitelist/blacklist evaluation, and large-file threshold handling (use existing mock provider/context helpers).
  - [ ] 1.2 Define new config globals in `autoload/vim_ai_config.vim` (enable toggle, debounce, whitelist/blacklist, large-file threshold, context lines) with documentation comments.
  - [ ] 1.3 Implement event hooks to start/reset the debounce timer after Normal mode insertions and cursor moves; ensure only "normal" buffers trigger requests.
  - [ ] 1.4 Reuse `py/context.py` to gather fill-in-the-middle context (configurable lines above/below cursor) and prepare provider payload; wire through existing provider abstraction to request a single completion.
  - [ ] 1.5 Ensure only the tests written in 1.1 pass (run targeted pytest module or functions); do not run the full suite.
...
Read @agent-os/specs/2025-10-13-ai-autocomplete/spec.md to understand the context for this spec and where the current task fits into it.
...
For example, if you've been assigned implement the 3rd task from `tasks.md` and that task's title is "Commenting System", then you must create the file: `agent-os/specs/2025-10-13-ai-autocomplete/implementation/3-commenting-system-implementation.md`.
...
## Overview
**Task Reference:** Task #[number] from `agent-os/specs/2025-10-13-ai-autocomplete/tasks.md`
